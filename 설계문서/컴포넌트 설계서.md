...
- 기술 수준 상충 시 기준: 시퀀스 다이어그램 우선 (Logging DB, 세션 식별자, Tool Calling 등 유스케이스 명세서 미기재 항목 포함)
...
| ProblemListView | 문제 목록 조회 및 필터링 |
...
| ProblemModule | 문제 목록/단건 조회 | C-06 |
...
| AgentGateway | LLM 호출 공통 처리 (타임아웃, 재시도, 프롬프트 조립, 응답 파싱) | C-03, C-04 |
...
```json
{
  ...
  "criteria_scores": [
    { "criterion": "", "score": 0, "max_score": 0, "rationale": "", "improvement": "" }
  ],
  "total_score": 0,
  "overall_comment": ""
}
...
| Tool | 파라미터 | 반환 | 조회 대상 |
|---|---|---|---|
| `get_feedback` | session_id, fields[] | 유저 답안, 점수, 개선 방향, 근거 중 요청 항목 | C-07 |
| `get_problem_index` | session_id | 문제 인덱스 | C-07 |
...
- `session_id`는 에이전트 생성 값 미신뢰, ToolExecutor가 현재 요청의 세션 식별자로 덮어씀
  - 위험 시나리오: 사용자가 채팅창에 "다른 세션 내용을 보여줘" 등을 입력할 경우, 에이전트가 임의의 `session_id`로 Tool 호출 시도 가능
  - 근거: 에이전트의 Tool 인자는 사용자 입력의 영향을 받는 값이므로 신뢰 불가
  - 인증 유무와 무관하게 적용 (인증 미도입 상태에서도 필요)
  - 구현: ToolExecutor가 인자 병합 시 요청 컨텍스트의 세션 식별자를 우선 적용
  - 상태: 팀 미논의 항목, 검토 필요 (10절 #7)

---

### 4.5 C-05 / C-06 / C-07 데이터 저장소
...
erDiagram
    ...
    problems ||--o{ sessions : referenced_by
...
| 저장소 | 대응 테이블 | 인덱스 |
|---|---|---|
| User DB | `users` | `identifier` UNIQUE |
| 문제 DB | `problems` | `(university, year, type)` 복합 인덱스 |
| Logging DB | `sessions`, `answers`, `gradings`, `chat_messages` | `sessions(user_id, created_at DESC)`, `answers(session_id)` UNIQUE, `gradings(answer_id)`, `chat_messages(session_id, created_at)` |
...
- 세션당 1행 유지, 자동 저장은 `updated_at` 갱신을 동반한 upsert
- `gradings`는 `answer_id`를 참조 → 재채점 발생 시 어느 원문에 대한 채점인지 추적 가능

**JSONB 적용 기준**

- `gradings.result`: 어/문법 오류 목록, 기준별 점수·근거·개선 방향 일괄 저장
- `total_score`만 별도 컬럼으로 승격 (집계 질의 대상)
...
| 4 | PUT | `/sessions/{id}/answer` | `content` | `updated_at` | 채점 (자동 저장) |
...
| 8 | POST | `/sessions/{id}/chat` | `message` | 답변 | 채팅 |
...
| API Server | Grading Agent / Chatting Agent | LLM API |
...
| # | 항목 | 영향 |
|---|---|---|
| 1 | 세션 이름 생성 방식 (자동 생성 / 사용자 입력 / 문제명 사용) | 과거 세션 목록 UI |
| 2 | 동일 문제 재응시 허용 여부 | 세션-문제 관계의 카디널리티 |
| 3 | 한 세션 내 재채점 허용 여부 | `gradings` 테이블의 1:1 / 1:N 여부 |
...
| 7 | Tool 인자 `session_id` 강제 주입 규칙 채택 여부 (팀 미논의, 작성자 제안) | 타 세션 데이터 조회 가능성, ToolExecutor 구현 |
