...
항목별 0~5점으로 평가하고, 각 점수의 구체적인 근거와 학생이 스스로 개선할 수 있는 방향을
제시하라. 개선 방향은 완성된 답안이나 문단을 대신 쓰지 말고 작성 방향만 안내하라.

점수는 다음 원칙을 따른다. 답안이 문제와 무관하거나 실질적인 답변이 아니면 0점을 부여하라.
특정 채점 기준에 대한 근거가 학생 답안에 전혀 없으면 해당 항목도 0점으로 평가하라. 1점은
최소한의 관련 내용은 있으나 매우 빈약한 경우에만 부여하라. 모범답안과 참고 자료는 학생
답안의 근거가 아니므로 절대로 대신 사용하지 마라.
...
점수 범위는 0~5점이며, 각 점수는 반드시 학생 답안에서 확인되는 근거를 바탕으로 결정하라.
...
해당 기준에 대한 근거가 전혀 없는 경우는 0점으로 설명하라.
